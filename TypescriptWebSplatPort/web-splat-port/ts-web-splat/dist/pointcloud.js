...
export class AabbImpl {
    ...
    static unit() {
        return new AabbImpl([-1, -1, -1], [1, 1, 1]);
    }
}
export class PointCloud {
    // GPU buffers - matches Rust struct fields
    splat2dBuffer; // splat_2d_buffer
    bindGroup; // bind_group
    renderBindGroup; // render_bind_group
    ...
    covarsBuffer;
    quantizationUniform;
    constructor(numPoints, shDeg, compressed, bbox, center, up, mipSplatting, kernelSize, backgroundColor) {
    ...
    static async new(device, pc) {
        ...
        let covarsBuffer;
        let quantizationUniform;
        // Handle compressed data (matches Rust compressed branch)
        if (pc.compressed()) {
            if (!pc.covars)
                throw new Error("Compressed point cloud missing covariances");
            if (!pc.quantization)
                throw new Error("Compressed point cloud missing quantization");
            ...
            bindGroupEntries.push({ binding: 3, resource: { buffer: covarsBuffer } }, { binding: 4, resource: { buffer: quantizationUniform.getBuffer() } });
        }
        ...
        const bbox = new AabbImpl([pc.aabb.min.x, pc.aabb.min.y, pc.aabb.min.z], [pc.aabb.max.x, pc.aabb.max.y, pc.aabb.max.z]);
        const backgroundColor = pc.background_color ?
            [pc.background_color[0], pc.background_color[1], pc.background_color[2], 1.0] :
            undefined;
        const self = new PointCloud(pc.num_points, pc.sh_deg, pc.compressed(), bbox, [pc.center.x, pc.center.y, pc.center.z], pc.up ? [pc.up.x, pc.up.y, pc.up.z] : undefined, pc.mip_splatting, pc.kernel_size, backgroundColor);
    ...
    isCompressed() { return this.compressed; }
    getNumPoints() { return this.numPoints; }
    getShDeg() { return this.shDeg; }
    getBbox() { return this.bbox; }
    getCenter() { return this.center; }
    getUp() { return this.up; }
    getMipSplatting() { return this.mipSplatting; }
    getDilationKernelSize() { return this.kernelSize; }
    getRenderBindGroup() { return this.renderBindGroup; }

...
async function parseZip(buf) {
    ...
    if (entries.size === 0)
        throw new Error("No ZIP entries found (invalid NPZ)");
    return entries;
}
...
async function unzipMaybe(compData, method, expectedSize) {
    if (method === 0) {
        return compData;
    }
    else if (method === 8) {
        ...
        const input = new Response(compData).body;
...
async function getFloat32Array(entries, keys, minDims) {
...
function parseNPYFloat32(buf) {
    ...
    if (!(descr === "<f4" || descr === "|f4"))
        throw new Error(`Unsupported dtype ${descr}, expected <f4`);
...
function pack2x16f(a, b) { const ha = f32tof16(a); const hb = f32tof16(b); return (hb << 16) | ha; }
...
function tripletsToDegree(triplets) {

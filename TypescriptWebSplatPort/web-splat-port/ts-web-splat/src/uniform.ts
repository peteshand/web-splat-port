export class UniformBuffer<T> {
  ...
  static newDefault<T>(device: GPUDevice, defaultValue: T, label?: string): UniformBuffer<T> {
  ...
  private writeDataToArray(array: Uint8Array, data: T): void {
  ...
  private writeObjectToView(view: DataView, obj: any, offset: number): number {
    for (const [key, value] of Object.entries(obj)) {
      if (typeof value === 'number') {

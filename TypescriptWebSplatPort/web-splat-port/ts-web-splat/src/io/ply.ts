// PLY reader for 3D Gaussian Splat format (binary_little_endian)
// Produces packed buffers matching WGSL expectations in preprocess.wgsl
...
export class PlyReader implements PointCloudReader {
  ...
  async read(): Promise<GenericGaussianPointCloud> {
    ...
    if (!vtx) throw new Error("PLY missing vertex element");
    ...
    for (let i = 0; i < vtx.properties.length; i++) pIndex[vtx.properties[i].name] = i;
    ...
    for (let i = 0; i < n; i++) {
      const off = i * stride;
      const f = (name: string) => dv.getFloat32(off + 4 * pIndex[name], true);
...
// ===== Header parsing =====
interface PlyProperty { name: string; type: string; }
interface PlyElement { name: string; count: number; properties: PlyProperty[]; }
interface PlyHeader { format: string; elements: PlyElement[]; }

function parseHeader(buf: ArrayBuffer): { header: PlyHeader; headerEnd: number } {
  ...
  search: for (let i = 0; i + needle.length <= u8.length && i < 1_000_000; i++) {
    ...
    if (ok) { pos = i; break search; }
  }
  if (pos < 0) throw new Error("PLY header not found (no end_header within first 1MB)");
  // Advance to end of line (LF) after end_header
  let end = pos;
  while (end < u8.length && u8[end] !== 0x0a) end++;
  if (end < u8.length) end++; // include LF
  const headerBytes = u8.slice(0, end);
  const text = new TextDecoder().decode(headerBytes);
  const lines = text.split(/\r?\n/);
  if (lines[0]?.trim() !== "ply") throw new Error("Missing ply magic");
  let i = 1;
  const header: PlyHeader = { format: "", elements: [] };
...
function pack2x16f(a: number, b: number): number {
  const ha = f32tof16(a);
  const hb = f32tof16(b);
  return (hb << 16) | ha;
}

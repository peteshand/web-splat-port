...
export class SceneCamera {
  ...
  static from_perspective(
    ...
    const q = quat.fromValues(
      cam.rotationQuat[0], cam.rotationQuat[1], cam.rotationQuat[2], cam.rotationQuat[3]
    );
    quat.normalize(q, q);
    const R = mat3.create();
    mat3.fromQuat(R, q);
...
export class Scene {
  ...
  static from_cameras(cameras: SceneCamera[]): Scene {
    ...
    if (cameras.length > 0) {
      let cx = 0, cy = 0, cz = 0;
      for (const c of cameras) { cx += c.position[0]; cy += c.position[1]; cz += c.position[2]; }
  ...
  static async from_json(file: ArrayBuffer): Promise<Scene> {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (!c.split) c.split = i < Math.floor(0.875 * n) ? Split.Train : Split.Test;
    }

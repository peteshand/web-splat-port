            ...
            WindowEvent::RedrawRequested => {
                ...
                unsafe {
                    ...
                    if FRAME_COUNT % 60 == 0 { // Log every 60 frames to avoid spam
                        log::info!("[RUST] Frame {} - Camera position: {:?}", FRAME_COUNT, state.splatting_args.camera.position());
                        log::info!("[RUST] Frame {} - Viewport: {:?}", FRAME_COUNT, state.splatting_args.viewport);
                        log::info!("[RUST] Frame {} - Point count: {}", FRAME_COUNT, state.pc.num_points());
                    }
                }
                
                let (request_redraw, shapes) = state.ui();
                let redraw_ui = shapes.viewport_output.values().any(|v| v.repaint_delay.is_zero());
                ...
                if request_redraw || redraw_ui{
                    ...
                    match state.render(request_redraw,state.ui_visible.then_some(shapes)) {
                        ...
                        Err(wgpu::SurfaceError::OutOfMemory) => {log::error!("out of memory");target.exit()},
                        // All other errors (Outdated, Timeout) should be resolved by the next frame
                        Err(e) => eprintln!("{:?}", e),
                    }
                }
                if !config.no_vsync {
                    let target_time = last + min_wait;
                    target.set_control_flow(ControlFlow::WaitUntil(target_time));
                }
            }
            _ => {}
        },

    ...
    pub fn prepare(
        ...
        unsafe {
            ...
            if FRAME_COUNT % 60 == 0 { // Log every 60 frames to avoid spam
                log::info!("[RUST] Frame {} - Camera view matrix: {:?}", FRAME_COUNT, render_settings.camera.view_matrix().x);
                log::info!("[RUST] Frame {} - Camera proj matrix: {:?}", FRAME_COUNT, render_settings.camera.proj_matrix().x);
                log::info!("[RUST] Frame {} - Viewport: {:?}", FRAME_COUNT, render_settings.viewport);
                log::info!("[RUST] Frame {} - Gaussian scaling: {}", FRAME_COUNT, render_settings.gaussian_scaling);
                log::info!("[RUST] Frame {} - Max SH deg: {}", FRAME_COUNT, render_settings.max_sh_deg);
            }
        }

...
impl<R: io::Read + io::Seek> PointCloudReader for PlyReader<R> {
    fn read(&mut self) -> Result<GenericGaussianPointCloud, anyhow::Error> {
        ...
        let limited_points = std::cmp::min(self.num_points, 1000);
        let mut gaussians = Vec::with_capacity(limited_points);
        let mut sh_coefs = Vec::with_capacity(limited_points);

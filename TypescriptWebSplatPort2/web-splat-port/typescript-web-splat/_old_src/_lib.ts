...
import { GaussianRenderer, SplattingArgs, Display } from './renderer.js';
import { PointCloud } from './pointcloud.js';
import { Camera, PerspectiveCamera, PerspectiveProjection } from './camera.js';
import { CameraController } from './controller.js';
import * as io from './io/mod.js';
import { Animation, TrackingShot, Transition } from './animation.js';
import { Scene, SceneCamera, Split } from './scene.js';
import { key_to_num, GPUStopwatch } from './utils.js';
...
export class WindowContext {
  ...
  static async new(
    ...
    const eye = {
      x: aabb.center().x - aabb.radius() * 0.5,
      y: aabb.center().y - aabb.radius() * 0.5,
      z: aabb.center().z - aabb.radius() * 0.5,
    };
    const view_camera = new PerspectiveCamera(
      eye,
      { x: 0, y: 0, z: 0, w: 1 }, // identity quaternion
      new PerspectiveProjection(vec2.fromValues(size.width, size.height), vec2.fromValues(45, 45 / aspect), 0.01, 1000)
    );
    ...
    if (this.animation) {
      ...
      if (this.current_view != null && this.scene) {
        ...
        if (cam) {
          ...
          const a = this.splatting_args.camera.position;
          const b = (scene_camera as any).position;
          const near = (x: number, y: number) => Math.abs(x - y) <= 1e-4;
          const pos_change = !(near(a.x, b.x) && near(a.y, b.y) && near(a.z, b.z));
          // Rotation equality omitted; treat as changed only if position differs
  ...
  private set_scene(scene: Scene): void {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (c) {
        acc.x += c.position.x; acc.y += c.position.y; acc.z += c.position.z;
        cnt++;
      }
    }
    ...
    if (this.saved_cameras.length === 0) {
      // Save “test” split cameras if such a concept exists; otherwise empty
  ...
  private start_tracking_shot(): void {
    if (this.saved_cameras.length > 1) {
      const shot = TrackingShot.fromCameras(this.saved_cameras.slice());
      const a = new Animation(this.saved_cameras.length * 2.0, true, shot as any);
  ...
  public set_camera(camera: PerspectiveCamera | SceneCamera, animation_duration: number): void {
    ...
    if (animation_duration <= 0) {
      ...
      const a = new Animation(
        animation_duration,
        false,
        new Transition(this.splatting_args.camera, target, smoothstep) as any
      );
      this.animation = [a, true];
    }
  }

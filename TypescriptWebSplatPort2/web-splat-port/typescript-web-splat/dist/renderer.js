...
export class GaussianRenderer {
    ...
    serializeCameraUniform(camera) {
        ...
        f32.set(camera.viewMatrix, 0); // [0..16)
        f32.set(camera.viewInvMatrix, 16); // [16..32)
        f32.set(camera.projMatrix, 32); // [32..48)
        f32.set(camera.projInvMatrix, 48); // [48..64)
        f32[64] = camera.viewport[0];
        f32[65] = camera.viewport[1]; // [64..66)
        f32[66] = camera.focal[0];
        f32[67] = camera.focal[1]; // [66..68)
    ...
    serializeSettingsUniform(u) {
        ...
        for (let i = 0; i < 4; i++)
            dv.setFloat32(off + i * 4, u.clippingBoxMin[i], true);
        off += 16;
        // clipping_box_max (vec4)
        for (let i = 0; i < 4; i++)
            dv.setFloat32(off + i * 4, u.clippingBoxMax[i], true);
        off += 16;
        ...
        // scene_center vec4
        for (let i = 0; i < 4; i++)
            dv.setFloat32(off + i * 4, u.sceneCenter[i] ?? 0, true);
        return new Uint8Array(buf);
    }
    ...
    prepare(encoder, device, queue, pc, renderSettings, stopwatch) {
        ...
        this.preprocess.run(encoder, pc, cameraBG, // group(0)

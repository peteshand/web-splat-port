...
class Internal {
   ...
   * Kept loose on the controller shape to avoid over-constraining types here.
   */
  public static function bind_input(canvas:CanvasElement, controller:Dynamic):Void->Void {
    ...
    var log = function(a:Dynamic, ?b:Dynamic, ?c:Dynamic, ?d:Dynamic, ?e:Dynamic) {
      if (DEBUG) js.Browser.console.debug("[input]", a, b, c, d, e);
    };

    var mapCode = function(code:String):Null<String> {
      return switch (code) {
    ...
    var updateAlt = function(e:Event) {
      // Mirror winit's modifier tracking by sampling altKey on each event
      try Reflect.setField(controller, "alt_pressed", (cast e).altKey == true) catch (_:Dynamic) {}
    };
    ...
    var onKeyDown = function(e:KeyboardEvent) {
      ...
      if (code == null) return;
      if (controller.process_keyboard != null && controller.process_keyboard(code, true)) {
        log("keydown", code);
        e.preventDefault();
      }
    };
    var onKeyUp = function(e:KeyboardEvent) {
      ...
      var code = mapCode(e.code);
      if (code == null) return;
      if (controller.process_keyboard != null && controller.process_keyboard(code, false)) {
        log("keyup", code);
        e.preventDefault();
      }
    };
    ...
    var onPointerDown = function(e:PointerEvent) {
      ...
      if (e.button == 0) Reflect.setField(controller, "left_mouse_pressed", true);
      if (e.button == 2) Reflect.setField(controller, "right_mouse_pressed", true);
      log("pointerdown", e.button, "alt=", Reflect.field(controller, "alt_pressed"));
      e.preventDefault();
    };
    var onPointerMove = function(e:PointerEvent) {
      ...
      if (controller.process_mouse != null) {
    ...
    var onPointerUp = function(e:PointerEvent) {
      ...
      if (e.button == 0) Reflect.setField(controller, "left_mouse_pressed", false);
      if (e.button == 2) Reflect.setField(controller, "right_mouse_pressed", false);
    ...
    var onWheel = function(e:WheelEvent) {
      updateAlt(e);
      if (controller.process_scroll != null) controller.process_scroll(e.deltaY / 100);
      log("wheel", e.deltaY);
    ...
    var onWindowBlur = function(_){
      Reflect.setField(controller, "left_mouse_pressed", false);
      Reflect.setField(controller, "right_mouse_pressed", false);
    };

    // Attach
    js.Browser.window.addEventListener("keydown", onKeyDown, true);
    js.Browser.window.addEventListener("keyup", onKeyUp, true);
    js.Browser.window.addEventListener("blur", onWindowBlur);
    ...
    return function() {
      ...
      js.Browser.window.removeEventListener("keydown", onKeyDown, true);
      js.Browser.window.removeEventListener("keyup", onKeyUp, true);
      js.Browser.window.removeEventListener("blur", onWindowBlur);

...
import gpu.GPUSorter.PointCloudSortStuff;
import pointcloud.PointCloud;
import renderer.CameraUniform;
import renderer.PreprocessPipeline;
...
class GaussianRenderer {
  ...
  public static function create(
    ...
    return GPUSorter.create(device, queue).then(sorter -> {
      ...
      final url = new js.html.URL('./shaders/gaussian.wgsl', js.Browser.window.location.href).href;
      renderer.Internal.logi('[shader::gaussian url]', url);
  ...
  function preprocessStep(
    ...
    final V = renderer.Internal.mat4ColMajorToArray(cu.viewMatrix);
    final P = renderer.Internal.mat4ColMajorToArray(cu.projMatrix);
    ...
    renderer.Internal.logi('[preprocess]', 'view=' + renderer.Internal.fmtF32Slice(V));
    renderer.Internal.logi('[preprocess]', 'proj=' + renderer.Internal.fmtF32Slice(P));
    renderer.Internal.logi('[preprocess]', 'viewProj=' + renderer.Internal.fmtF32Slice(VP));
  ...
  public function prepare(
    ...
    if (this.sorterStuff == null || this.sorterStuff.numPoints != pc.numPoints()) {
      ...
      renderer.Internal.logi('[prepare]', 'sorter buffers (num_points=' + ss.numPoints + '): uni.size=' + ss.sorterUni.size + ' dis.size=' + ss.sorterDis.size);
    }
